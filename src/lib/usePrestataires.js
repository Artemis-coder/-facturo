...
const PRESTATAIRE_SELECT = "id, nom, societe, email, telephone, notes, type_projet, type_contrat, created_by, created_at, updated_at";
...
export function usePrestataires(entrepriseId, userId) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: prestataireRows, error: prestataireError }, { data: lienRows, error: lienError }] = await Promise.all([
    ...
    if (!prestataireError) setPrestataires((prestataireRows || []).map(mapPrestataire));
    if (!lienError) setLiens((lienRows || []).map(mapLien));
    setLoading(false);
  }, [entrepriseId]);
  ...
  return { prestataires, liens, loading, savePrestataire, deletePrestataire, affecterPrestataire, detacherPrestataire, reload: load };
}
